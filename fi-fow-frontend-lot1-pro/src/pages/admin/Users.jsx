...
import { Ban, BadgeX, RotateCcw, ShieldAlert } from 'lucide-react'
...
export default function AdminUsers() {
  ...
  return (
    <AdminPage eyebrow="Comptes et accès" title="Utilisateurs" description="Recherchez un compte et appliquez uniquement les mesures nécessaires et proportionnées.">
      ...
      {rows.length ? <AdminTable columns={columns} rows={rows} mobileTitle={(row) => row.fullName} mobileSubtitle={(row) => `${row.email} · ${roleLabel(row.role)}`} mobileMeta={(row) => <AdminStatusBadge status={row.status} />} actions={(row) => <UserActions row={row} currentUser={auth.user} onAction={setDialog} />} /> : null}
...
function UserActions({ row, currentUser, onAction }) {
