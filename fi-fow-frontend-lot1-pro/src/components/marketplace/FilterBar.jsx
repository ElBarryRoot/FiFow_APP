...
const locations = ['Kaloum', 'Dixinn', 'Matam', 'Ratoma', 'Matoto']
        ...
        <FilterGroup title="État du produit">
          <div className="space-y-2">
            {conditions.map((condition) => (
              <label key={condition.value} className="flex cursor-pointer items-center gap-3 text-sm font-semibold text-fifow-secondary">
                <input type="radio" name="condition" checked={filters.condition === condition.value} onChange={() => onChange('condition', condition.value)} className="h-4 w-4 accent-fifow-primary" />
                {condition.label}
              </label>
            ))}
            {filters.condition ? (
