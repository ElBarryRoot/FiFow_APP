...
export function orderProduct(order) {
  ...
  return {
    ...
    image: resolveMediaUrl(mainImage) || '/assets/empty-product.svg',
    price: Number(order?.itemAmount || product.price || snapshot.price || 0),
    listingMode: product.listingMode || snapshot.listingMode || 'SINGLE',
    lotItemCount: product.lotItemCount ?? snapshot.lotItemCount ?? null,
  }
}
