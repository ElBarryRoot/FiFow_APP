...
import { useFormattedGNFInput } from './moneyInput.js'
import ListingModeSelector from './ListingModeSelector.jsx'

export default function ProductPriceLocationStep({ draft, updateDraft, errors = {}, canManageStock = false }) {
  const quartiers = quartiersByCommune[draft.commune] || []
  const priceInput = useFormattedGNFInput({
    value: draft.price,
    onValueChange: (price) => updateDraft({ price }),
  })
  ...
  return (
    <div>
      ...
      <div className="mt-7 grid items-end gap-5 sm:grid-cols-[minmax(0,1fr)_auto]">
        <FormField label={draft.listingMode === 'DONATION' ? 'Valeur de l’article' : 'Prix'} hint={draft.listingMode === 'DONATION' ? 'Le don est gratuit : aucun paiement ne sera demandé.' : 'Les espaces sont ajoutés automatiquement.'} error={errors.price}><div className="flex h-14 overflow-hidden rounded-lg border border-fifow-border bg-white transition focus-within:border-fifow-primary focus-within:ring-4 focus-within:ring-violet-100"><input ref={priceInput.inputRef} value={priceInput.displayValue} onChange={priceInput.onChange} disabled={draft.listingMode === 'DONATION'} placeholder={draft.listingMode === 'DONATION' ? '0' : 'Ex. 500 000'} inputMode="numeric" aria-label="Prix en francs guinéens" aria-invalid={Boolean(errors.price)} className="min-w-0 flex-1 px-4 text-base font-bold text-fifow-dark outline-none placeholder:text-fifow-muted disabled:bg-slate-50 disabled:text-fifow-secondary" /><span className="grid w-20 place-items-center border-l border-fifow-border bg-slate-50 text-sm font-extrabold text-fifow-dark">GNF</span></div></FormField>
      ...
      <div className="mt-8"><ListingModeSelector value={draft.listingMode} quantity={draft.stockQuantity} lotItemCount={draft.lotItemCount} onChange={updateDraft} canManageStock={canManageStock} error={errors.stockQuantity} lotError={errors.lotItemCount} /></div>
