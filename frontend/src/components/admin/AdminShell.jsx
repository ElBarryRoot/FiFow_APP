...
const navigation = [
  {
    ...
    items: [
      ...
      { label: 'Litiges', to: '/admin/disputes', icon: MessageSquareWarning },
...
// Parcours administrateur : supervision, opérations, confiance, finance, configuration.
// Les entrées réutilisent la définition existante pour conserver leurs permissions et routes.
const navigationByRoute = new Map(navigation.flatMap((section) => section.items.map((item) => [item.to, item])))
const navigationItem = (to) => navigationByRoute.get(to)
const orderedNavigation = [
  { label: 'Pilotage', items: ['/admin'].map(navigationItem) },
  { label: 'Operations marketplace', items: ['/admin/products', '/admin/orders', '/admin/users', '/admin/conversations', '/admin/reviews'].map(navigationItem) },
  { label: 'Confiance et support', items: ['/admin/disputes', '/admin/reports', '/admin/verifications', '/admin/support'].map(navigationItem) },
  { label: 'Finance', capability: 'manageFinance', items: ['/admin/payments', '/admin/payouts', '/admin/boosts'].map(navigationItem) },
  { label: 'Configuration', capability: 'manageCatalogue', items: ['/admin/categories', '/admin/logs', '/admin/settings'].map(navigationItem) },
]
...
export default function AdminShell() {
  ...
  const dashboardQuery = useQuery({ queryKey: ['admin', 'dashboard'], queryFn: adminApi.dashboard, refetchInterval: 60_000 })
  const openDisputes = dashboardQuery.data?.openDisputes || 0
                ...
                <div className="space-y-1">
                  {visibleItems.map((item) => (
                    ...
                    >
                      ...
                      {['/admin/orders', '/admin/disputes'].includes(item.to) && openDisputes > 0 ? <span className="grid min-w-5 h-5 place-items-center rounded-full bg-red-600 px-1.5 text-[11px] font-black text-white">{openDisputes > 99 ? '99+' : openDisputes}</span> : null}
                    </NavLink>
                  ))}
                </div>
              </div>
