...
import { orderedItemLabel } from '../../lib/listingAvailability.js'
...
export default function CheckoutProductSummary({ order, compact = false }) {
  ...
  return (
    <Card className="overflow-hidden p-5 sm:p-6">
      {order.items?.length > 1 ? <div><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-black text-fifow-dark">{order.items.length} articles</h2><p className="text-lg font-black text-fifow-primary">{formatGNF(order.itemAmount)}</p></div><div className="mt-3 divide-y divide-fifow-border">{order.items.map((item) => <div key={item.id} className="flex items-center gap-3 py-3"><img src={item.product.image} alt="" className="h-14 w-14 rounded-lg object-cover" /><div className="min-w-0 flex-1"><p className="line-clamp-1 text-sm font-black text-fifow-dark">{item.product.title}</p><p className="mt-1 text-xs font-bold text-fifow-secondary">{orderedItemLabel(item.product, item.quantity)} · {itemPriceLabel(item)}</p></div><p className="text-sm font-black text-fifow-dark">{formatGNF(item.lineTotal)}</p></div>)}</div><p className="mt-3 flex items-center gap-2 text-sm font-bold text-fifow-secondary"><Store className="h-4 w-4 shrink-0 text-fifow-primary" /> Vendeur : {order.sellerName || 'Vendeur Fi Fow'}</p></div> : <div className="flex gap-4">
        <img
          src={product.image}
          alt={product.title}
          width="112"
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1.5 text-xs font-extrabold text-fifow-secondary">{orderedItemLabel(firstItem?.product || product, firstItem?.quantity || 1)}</p>
...
function itemPriceLabel(item) {
  const price = formatGNF(item.unitPrice)
  if (item.product?.listingMode === 'LOT') return `${price} le lot`
  if (item.product?.listingMode === 'STOCK') return `${price} l’exemplaire`
  return price
}
