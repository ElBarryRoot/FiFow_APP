import { Boxes, Eye, Gift, Heart, Layers3, MapPin, ShieldCheck, Zap } from 'lucide-react'
...
export default function ProductCard({ product, compact = false, horizontal = false }) {
  ...
  const InventoryIcon = product.listingMode === 'DONATION' ? Gift : product.listingMode === 'LOT' ? Layers3 : Boxes
  const showInventory = ['LOT', 'STOCK', 'DONATION'].includes(product.listingMode)
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-[150px] flex-1 flex-col p-4">
        ...
        {showInventory ? (
          <p className="mt-1.5 flex items-center gap-1.5 text-xs font-extrabold text-fifow-secondary">
            <InventoryIcon className="h-3.5 w-3.5 shrink-0 text-fifow-primary" />
            <span className="truncate">{listingAvailabilityLabel(product)}</span>
          </p>
        ) : null}
