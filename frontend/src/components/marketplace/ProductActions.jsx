...
import { Flag, Gift, Heart, MessageCircle, ShoppingBag, ShoppingCart, ThumbsUp } from 'lucide-react'
  ...
  return (
    <div className="space-y-3 lg:space-y-4">
      <div className="grid grid-cols-2 gap-2 lg:grid-cols-3 lg:gap-3">
        {product.listingMode === 'DONATION' ? <Button type="button" onClick={() => openConversation('donation')} loading={contactMutation.isPending} icon={Gift} size="lg" className="col-span-2 w-full lg:col-span-3">Contacter pour le don</Button> : <><Button type="button" onClick={startPurchase} icon={ShoppingBag} size="lg" className="w-full">Acheter</Button><Button type="button" onClick={addToCart} loading={cartMutation.isPending} icon={ShoppingCart} size="lg" variant="secondary" className="w-full">Au panier</Button></>}
        <Button type="button" onClick={() => openConversation('contact')} loading={contactMutation.isPending} icon={MessageCircle} size="lg" variant="ghost" className="col-span-2 w-full border border-fifow-border lg:col-span-1">Contacter le vendeur</Button>
      </div>
