...
function validateStep(step, draft) {
  ...
  if (step === 3) {
    if (draft.listingMode === 'DONATION' ? !/^0$/.test(draft.price) : !/^[1-9][0-9]{2,14}$/.test(draft.price)) errors.price = draft.listingMode === 'DONATION' ? 'Pour un don, indiquez 0 GNF.' : 'Indiquez un prix entier d’au moins 100 GNF.'
    ...
    if (draft.listingMode === 'LOT' && (!Number.isInteger(Number(draft.lotItemCount)) || Number(draft.lotItemCount) < 2 || Number(draft.lotItemCount) > 10000)) errors.lotItemCount = 'Indiquez combien d’articles composent le lot, entre 2 et 10 000.'
  }
  return errors
}
  ...
  async function publishProduct() {
    ...
    try {
      const productInput = {
        ...
        lotItemCount: draft.listingMode === 'LOT' ? Number(draft.lotItemCount) : null,
