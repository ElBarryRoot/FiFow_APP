...
export default function EditListing() {
  ...
  const [lotItemCount, setLotItemCount] = useState(null)
  ...
  useEffect(() => {
    ...
    setLotItemCount(listing.listingMode === 'LOT' ? listing.lotItemCount ?? 2 : null)
  }, [listing])
  ...
  function formInput() {
    ...
    return {
      ...
      lotItemCount: listingMode === 'LOT' ? Number(lotItemCount) : null,
    }
  }
  ...
  async function save(event) {
    ...
    if (listingMode === 'LOT' && (!Number.isInteger(Number(lotItemCount)) || Number(lotItemCount) < 2 || Number(lotItemCount) > 10000)) {
      inventoryErrors.lotItemCount = 'Indiquez combien d’articles composent le lot, entre 2 et 10 000.'
    }
    if (listingMode === 'STOCK' && (!Number.isInteger(Number(stockQuantity)) || Number(stockQuantity) < 1 || Number(stockQuantity) > 10000)) {
      inventoryErrors.stockQuantity = 'Indiquez une quantité comprise entre 1 et 10 000.'
    }
    if (Object.keys(inventoryErrors).length) {
      setFieldErrors(inventoryErrors)
      return false
    }
    setFieldErrors({})
    try {
    ...
    <UserPageShell title="Modifier l’annonce" subtitle="Mettez à jour le brouillon avant de le soumettre." backTo="/profile/listings" backLabel="Retour à mes annonces">
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <Card as="form" ref={formRef} onSubmit={save} className="space-y-5 p-5 sm:p-7">
          ...
          <ListingModeSelector value={listingMode} quantity={stockQuantity} lotItemCount={lotItemCount} canManageStock={Boolean(auth.user?.canManageStock)} error={fieldErrors.stockQuantity} lotError={fieldErrors.lotItemCount} onChange={(next) => { if (next.listingMode !== undefined) setListingMode(next.listingMode); if (next.stockQuantity !== undefined) setStockQuantity(next.stockQuantity); if (next.lotItemCount !== undefined) setLotItemCount(next.lotItemCount) }} />
