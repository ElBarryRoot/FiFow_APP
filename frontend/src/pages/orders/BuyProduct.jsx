...
import { listingAvailabilityLabel, orderedItemLabel } from '../../lib/listingAvailability.js'
    ...
    <Card className="p-4">
      {cartGroup ? (
        <div>
          ...
          <div className="mt-3 divide-y divide-fifow-border">{cartGroup.items.map((item) => <div key={item.id} className="flex gap-3 py-3"><img src={item.product.imageUrl || '/assets/empty-product.svg'} alt="" className="h-14 w-14 rounded-lg object-cover" /><div className="min-w-0"><p className="line-clamp-1 text-sm font-black text-fifow-dark">{item.product.title}</p><p className="mt-1 text-xs font-bold text-fifow-secondary">{orderedItemLabel(item.product, item.quantity)} · {formatGNF(item.currentUnitPrice)}</p></div></div>)}</div>
