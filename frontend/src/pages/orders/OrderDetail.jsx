...
export default function OrderDetail() {
  ...
  return (
    <UserPageShell title={product.title || 'Commande'} eyebrow={`Commande ${order.reference}`} subtitle={status.label} actions={<Badge variant={status.tone}>{status.shortLabel}</Badge>} backTo="/orders" backLabel="Commandes">
      ...
      <ReasonDialog open={Boolean(reasonAction)} title={reasonAction === 'cancel' ? 'Annuler cette commande ?' : 'Signaler un problème avec cette commande'} description={reasonAction === 'cancel' ? 'Cette action retire la commande avant paiement. Expliquez brièvement la raison.' : 'Décrivez les faits précisément. Fi Fow examinera la situation avant de décider du remboursement ou du versement au vendeur.'} confirmLabel={reasonAction === 'cancel' ? 'Confirmer l’annulation' : 'Envoyer le signalement'} danger loading={actionMutation.isPending} onClose={() => setReasonAction(null)} onConfirm={(reason) => actionMutation.mutate({ action: reasonAction, reason })} />
    </UserPageShell>
  )
}
