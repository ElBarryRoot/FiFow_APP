...
export function toProductView(product) {
  ...
  const images = product.images?.map((image) => resolveMediaUrl(image.url)).filter(Boolean) || []
  const mainImage = resolveMediaUrl(product.mainImage?.url) || images[0] || defaultProductImage
  return {
    ...product,
    id: product.id,
    ...
    seller: product.seller ? {
      ...
      avatar: resolveMediaUrl(product.seller.avatarUrl) || defaultAvatar,
...
export function toUserView(user) {
  ...
  return {
    ...
    avatar: resolveMediaUrl(user.avatarUrl) || defaultAvatar,
